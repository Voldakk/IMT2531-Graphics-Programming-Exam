...
namespace EVA
{
	...
	void EditorWindows::Inspector()
	{
		...
		case Skybox:
			...
			break;
		case Material:
			MaterialInspector(windowSize.x);
			break;
		default:
			break;
		}
	...
	void EditorWindows::AssetBrowser()
	{
		...
		for (auto p : FS::directory_iterator(m_SelectedAssetFolder))
		{
			...
			else
			{
				...
				{
					if (p.path().has_extension() && p.path().extension().string() == ".mat")
					{
						SelectMaterial(MaterialManager::LoadMaterial(p).get());
					}
				}
				if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_SourceAllowNullID))
				{
	...
	void EditorWindows::SelectMaterial(EVA::Material* material)
	{
		m_SelectedType = Material;
		m_SelectedMaterial = material;
	}

	bool EditorWindows::IsSelected(EVA::Material* material) const
	{
		return m_SelectedType == Material && m_SelectedMaterial == material;
	}

	EVA::Material* EditorWindows::SelectedMaterial() const
	{
		return m_SelectedType == Material ? m_SelectedMaterial : nullptr;
	}
	...
	void EditorWindows::MaterialInspector(float width) const
	{
